...
const createAccount = async (req, res) => {
    ...
    db.query(checkSql, async (err, result) => {
        ...
        // Insert new admin
        const sql = "INSERT INTO admin(fname, lname, email, password, access_code) VALUES (?,?,?,?,?)";
...
const loginAccount = (req, res) => {
  ...
  db.query("SELECT * FROM admin WHERE email = ?", [email], async (err, results) => {
    ...
    if (!match) {
      ...
      db.query("UPDATE admin SET locked = 1 WHERE email = ?", [email], (err2, result) => {
    ...
    db.query(sql, [email], async (err, results) => {
        ...
        try {
            ...
            if (!match) {
                ...
                const lockSql = "UPDATE admin SET isLocked = 1 WHERE id = ?";
